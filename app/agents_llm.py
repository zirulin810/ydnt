...
creator_verify = LlmAgent(
    ...
    instruction=(
        ...
        "- If no creator is provided (the creator field in the course profile is empty or blank), do not attempt to guess, discover, or search who the creator is. Do not call any tools. Directly report it as unverifiable by returning: footprint='weak', verifiable_track_record=False, only_sells_courses=False, evidence_links=[].\n"
        "\n"
        ...
        "set `footprint` to 'strong' or 'medium', and set `verifiable_track_record` to True (interpreted as a 'verifiable, reputable real-world entity').\n"
...
verdict_agent = LlmAgent(
    ...
    instruction=(
        ...
        "The rubric result also has 'coverage_basis': 'video' or 'transcript' means the free alternative's coverage was "
        "checked against its actual content; 'metadata' means it was only estimated from titles and descriptions. "
        "When it is 'metadata', say in money_vs_time that the coverage figure is an estimate.\n"
