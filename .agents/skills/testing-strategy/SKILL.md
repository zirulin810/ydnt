...
description: Testing strategy for the YDNT project: test layers and evaluation targets. Use when writing tests or running evaluations.
version: 1.0.0

## Test Layers
1. **Unit (pytest)**: Focuses on pure logic in `nodes.py` (e.g. sentence stripping, budget logic) and `score.py`.
2. **Integration**: Uses `agents-cli run` to verify the entire DAG workflow executes from end-to-end.
...
## API Quota
- All tools call live APIs. Unit tests must stub network calls (monkeypatch `httpx`) instead of hitting YouTube or Jina.
- End-to-end runs spend YouTube and Gemini quota; run them deliberately.
