...
def test_finalize_verdict_success() -> None:
    ...
    ctx = MockContext(state={"verdict": verdict_data, "free_alternatives": free_alts})
    ...
    assert (
        output["free_alternatives"][0]["url"]
        == "https://youtube.com/watch?v=specific123"
    )
    assert (
        ctx.state["verdict"]["free_alternatives"][0]["url"]
        == "https://youtube.com/watch?v=specific123"
    )
...
def test_finalize_verdict_empty() -> None:
    ...
    ctx = MockContext(state={"verdict": verdict_data})
